...
def signup(user: UserCreate, db: Session = Depends(get_db)):
    ...
    new_user = User(email=user.email, password=hashed_pw, name=user.name, username=user.username)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    return {"msg": "Signup successful", "user": {"name": new_user.name, "username": new_user.username, "email": new_user.email}}
...
@router.put("/profile")
def update_profile(update: UserUpdate, db: Session = Depends(get_db), authorization: str = Header(None)):
    ...
    db.commit()
    db.refresh(user)
    return {"msg": "Profile updated", "user": {"name": user.name, "username": user.username, "email": user.email}}
